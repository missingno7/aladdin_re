"""Qualify one recorded 1B524E/1B5256 spawn-region entry without a rebuild."""
...
def run(fixture: Path, output: Path, *, entry: int | None = None) -> dict:
    ...
    with Machine(rom) as original:
        ...
        if entry not in SPAWN_REGION_ENTRIES or registers['pc'] != entry:
            raise ValueError('fixture must be parked at a recovered spawn-region entry')
        sp = registers['a7']
        return_pc = int.from_bytes(original.peek_ram(sp & 0xffff, 4), 'big') & 0xffffff
        initial = artifacts.snapshot_bytes(original)
        recorder = artifacts.Recorder(original, origin='synthetic', reset_provenance='spawn-region-witness')
    ...
    with Machine(rom) as suffix:
        ...
        recorder = artifacts.Recorder(suffix, origin='synthetic', reset_provenance='spawn-region-safe-exit')
        suffix.gates([])

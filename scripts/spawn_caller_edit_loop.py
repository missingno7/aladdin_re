"""Verify a disposable reverse-spawn correction edit changes a focused verdict."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix='aladdin-spawn-edit-') as temporary:
        ...
        needle = 'read(record + 2, 2) + 8'
        if code.count(needle) != 1:
            raise RuntimeError('spawn caller edit loop no longer identifies the x correction')
        lifecycle.write_text(code.replace(needle, 'read(record + 2, 2) + 9'))
        after = compare('after')
    ...
    report = {'status': 'PASS' if passed else 'FAIL', 'before': before, 'after': after,
              'edit': 'reverse-spawn x correction +8 -> +9 in a disposable source copy',

# Aladdin grinder goal prompt

Paste the block below as the goal of a long-running worker session on the
Aladdin candidate grind.  It is written for a cheaper model.  The supervising
session watches the ledger and the blocker packages; it does not need
progress prose.  The grind is paused (`STATUS.md`); the "first supervised
targets" section at the end is the historical list of 14 September, all
recovered since, and must be re-derived from a current frontier ledger.
The Gods worker has its own prompt (`../gods/grinder-goal.md`).
...
escalation condition.  Follow `docs/common/recovery-process.md` (the loop)
and `docs/aladdin/recovery-playbook.md` (Aladdin's recipes, tools, cadence,
commit rules, escalation codes and blocker package) exactly;
...
Start by reading, in this order and nothing else: the loop, the playbook,
`docs/aladdin/STATUS.md`, and the last twenty lines of
`docs/aladdin/ledger.md`.  Then run the frontier ledger on the newest PASS
...
## First supervised targets (historical, 14 September 2026)

Chosen from the frontier ledger of the newest PASS cold run named in
`docs/aladdin/ledger.md` and the evidence index in `artifacts/evidence/main`

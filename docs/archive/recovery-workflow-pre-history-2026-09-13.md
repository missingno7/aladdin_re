# Frozen historical evidence — pre-history workflow, 13 September 2026

This report is preserved for the measured recovery process and decisions at its
source freeze.  It is not current operating guidance: present commands use
immutable cold-start input histories and do not promise to load the legacy
replay or snapshot artifacts named below.
...
- `scripts/return_region_witness.py`: the existing strict sequence, shared by
  four small region wrappers. Each wrapper supplies allowed entries and capture
...
- `scripts/semantic_edit_check.py`: disposable source-copy comparison with an
  exactly-once semantic replacement. The four wrappers retain the actual
...
The latest original-only 1AE3FC capture now includes nonempty activations from
all six recordings, with up to two callbacks in a captured activation. See
`artifacts/grinding/luna/dispatcher-1ae3fc/nonempty-summary.json` and its map.
This establishes observed control flow, not recovered whole-iteration ownership.
A frontier listing may derive observed targets
...
## Subsequent reuse and current audit

The 1B7262 upper-variant caller reused both shared tools through small wrappers;
it did not introduce a new qualification protocol. Its strict witnesses cover
both recorded return sites. This is a concrete fourth use of the ordinary-return
contract, rather than a hypothetical abstraction. The receipt-audit tests were
rerun during the workflow review: 16 passed in 0.06 seconds.

The dispatcher call/restore work resumed after this review. Its initial
focused checks do not constitute full integration qualification. Existing
published replay receipts describe their frozen source versions, not this WIP.
No full replay rerun is claimed for this documentation review.

# Frozen historical evidence — pre-history status, 13 September 2026

This report is retained for its ROM maps, fixture results, and recovery
decisions at the recorded source freeze.  It is not current operating guidance:
the present player uses immutable cold-start input history and does not promise
to load the legacy replay or snapshot artifacts named below.
...
[source-guide.md](../../aladdin/source-guide.md) for the source/adapter boundary.
...
See [ownership-boundary.md](../project/ownership-boundary.md) for the ownership/observability
...
The [current architecture review](../project/architecture-review.md) rechecked the actual
...
## Scripted upper-pool callback

`1B72D4..1B72FA` now composes the upper allocator with type `0x34`, scripts
`0x122C1E` and `0x1217B4`, and mode flag `6`. Direct recorded fixtures,
synthetic exhaustion, deadline/alignment rejection, a strict return witness,
and a controlled parent-dispatch iteration preserve its return, timing, and
150-instruction native continuation.

## Plain reverse-pool callback

`1B7232..1B723C` now loads template `1B79E0`, composes the reverse allocator,
and immediately returns. Recorded direct fixtures, reverse-pool exhaustion,
deadline/alignment checks, a strict return witness, and controlled parent
iteration verify its state, timing, and native continuation contracts.

## Plain upper-pool callback

`1B6F76..1B6F80` now loads template `1B80AC`, composes the upper allocator,
and returns without a caller suffix. Its direct fixtures, pool exhaustion,
deadline/alignment checks, strict return witness, and controlled parent
iteration preserve state, timing, and native continuation.

## Primary-pool guarded spawn callbacks

Three adjacent primary-pool callbacks now have bounded native replacements:
`1B738A` admits when both `FFF177` and `FFF178` are nonzero (type `0x3C`),
`1B73C2` admits when `FFF177` is zero (type `0x3E`), and `1B73F2` admits
when `FFF177` is nonzero and `FFF178` is zero (type `0x3F`). Each preserves
its branch-specific CCR, shared-RTS bookkeeping, allocator exhaustion exit,
saved outer return, script fields, flag, deadline fallback, and 150-instruction
native continuation. Their recorded direct fixtures qualified through fresh
strict return witnesses.

## Bounded spawn dispatcher iteration

The paragraph below records the initial three-target milestone, not the current
expanded working tree. See [current-state-review.md](current-state-review.md)
for its combined-source qualification and remaining review items.

The lifecycle candidate now owns one selected `1AE468` dispatcher iteration:
native lookup supplies `A4`, then the candidate saves the 15-register frame,
composes known `1B6802` / `1B7262` / `1B735E` callers, restores the frame, and
performs the `ADDA` / `ADDI` / `DBRA` tail. It resumes native code at `1AE44A`
or final `1AE47C`; the remaining loop and 59 other observed targets stay
native, with rejection before the save. Full scope, manual timing/CCR facts,
strict witness, controls, 1,217-test suite, and six current-source receipts
are in [spawn-dispatch-iteration.md](spawn-dispatch-iteration.md).

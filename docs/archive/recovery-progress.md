...
## Guarded primary spawn family `1B738A`, `1B73C2`, and `1B73F2`

The adjacent primary allocator callers are now recovered as three explicit
guard truth tables, all sharing the measured template `1B79B8`, primary
allocator entry, caller-owned script fields, flag, saved-stack return, and
allocator-failure tail. `1B738A` requires both `FFF177` and `FFF178` nonzero
and writes type `0x3C`; `1B73C2` rejects nonzero `FFF177` and writes type
`0x3E`; `1B73F2` rejects zero `FFF177` or nonzero `FFF178` and writes type
`0x3F`. Direct tests exercise each guard, exhaustion, deadline, alignment,
outer state, and 150-instruction native future; strict witnesses use their
recorded dispatcher fixtures.

## Scripted upper allocator callback `1B72D4`

`1B72D4` loads template `1B79B8`, calls the established upper allocator, and
on success installs type `0x34`, scripts `0x122C1E`/`0x1217B4`, and flag `6`.
The recovered boundary retains the shared exhausted-path RTS, allocator CCR,
saved outer return, and exact suffix timing. Six recorded direct fixtures and
synthetic exhaustion compare outer state and a 150-instruction native future;
a strict direct witness and controlled dispatcher-iteration composition also
pass. A captured parent iteration for this target remains future coverage.

## Plain reverse allocator callback `1B7232`

`1B7232` is the direct reverse-pool wrapper around template `1B79E0` and the
established allocator entry. It has no caller-owned success suffix: allocator
state and CCR flow straight to the measured RTS. Recorded direct fixtures,
synthetic exhaustion, deadline and stack-alignment guards compare full outer
state and native continuation; strict direct and controlled parent-iteration
checks qualify the bounded replacement. A captured parent iteration remains
future coverage.

## Plain upper allocator callback `1B6F76`

`1B6F76` is the matching upper-pool wrapper, loading template `1B80AC` before
the established allocator and then returning directly. Its bounded replacement
preserves allocator residue and saved-stack return; direct fixtures, synthetic
pool exhaustion, deadline/alignment rejection, strict direct witness, and a
controlled parent iteration compare full outer state and native continuation.
A captured parent iteration remains future coverage.
...
## Selected spawn dispatcher iteration

Historical three-target qualification follows. Later caller additions expanded
the working tree to 16 directly composed targets; its receipts must be checked
against that source separately. See [current-state-review.md](../current-state-review.md).

`1AE468..1AE47A` now owns one selected callback iteration, including the
native 15-register save/restore and its `ADDA` / `ADDI` / `DBRA` tail. It
accepts only already recovered `1B6802`, `1B7262`, and `1B735E` targets, resumes
the original lookup at `1AE44A` or the final dispatcher RTS at `1AE47C`, and
refuses other targets before the save. The full 16-slot loop remains native:
six captures contain 62 targets. Strict original iteration fixtures, aliases,
deadline, DBRA/CCR controls, explicit PC/A7 fresh witness, edit loop, 1,217
tests, and all six current-source replay receipts are recorded in
[spawn-dispatch-iteration.md](spawn-dispatch-iteration.md).

        ...
        with Machine(rom) as machine:
            try:
                ...
                if recording:
                    ...
                    def save_recording():
                        path = filename(".alreplay")
                        data = recording.finish(
                            machine,
                            terminal_status="failed" if failed else "completed",
                            failure=describe(primary_error) if failed else None,
                            terminal_tick=last_completed_tick if failed else None,
                        )
                        artifacts.write_new(path, data)
                        print(f"Saved {'failed prefix ' if failed else ''}{path}", flush=True)

                    cleanup("recording save", save_recording)

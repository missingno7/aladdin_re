...
from aladdin_sega.profile import ALADDIN, ROOT_ID, read_rom, DEFAULT_ROM
...
def test_tree_reuses_prefix_and_matches_each_continuous_cold_leaf(tmp_path):
    ...
    tree = execute_history(ALADDIN, store, read_rom(), tree=True)
    assert tree['executed_frames'] == 9  # 2+2+2+3, instead of 6+7.
    assert len(store.nodes()) == 5
    for leaf in (c, x):
        cold = execute_history(ALADDIN, store, read_rom(), node=leaf)
...
def test_fresh_worker_tree_comparison(tmp_path):
    ...
    result = compare_history(ALADDIN, store.path, DEFAULT_ROM, candidate='original', tree=True,
...
def test_missing_terminal_evidence_fails_even_if_both_workers_omit_it(tmp_path, field):
    ...
    result = execute_history(ALADDIN, store, read_rom(), node=c)
...
def test_reference_and_candidate_workers_run_at_the_same_time(tmp_path):
    ...
    result = compare_history(ALADDIN, store.path, DEFAULT_ROM, candidate='original', output=tmp_path / 'c',
...
def test_sequential_mode_stops_at_the_first_failed_worker(tmp_path):
    ...
    result = compare_history(ALADDIN, store.path, DEFAULT_ROM, candidate='lifecycle', output=tmp_path / 'c',
...
def test_parallel_cold_comparison_of_a_real_branch_passes(tmp_path):
    store, _, _, c, _ = graph(tmp_path)
    result = compare_history(ALADDIN, store.path, DEFAULT_ROM, node=c, candidate='original', output=tmp_path / 'c')
    assert result['status'] == 'PASS', result
    assert result['workers'] == 'parallel'

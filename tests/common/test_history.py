...
def test_genesis_run_counts_logical_frames_despite_native_tick_overshoot():
    OBSERVATION_OFFSET_TICKS = GAME.observation_offset_ticks
    with GenesisRun(GAME, synthetic_rom()) as run:
        run.step(1)
        assert run.frame == 1
...
def test_corrupt_cache_is_ignored_and_can_be_regenerated(tmp_path):
    ...
    with GenesisRun(GAME, GAME.read_rom()) as writer:
    ...
    with GenesisRun(GAME, GAME.read_rom()) as reader:
    ...
    with GenesisRun(GAME, GAME.read_rom()) as restored:
...
def test_session_resume_records_input_release_at_next_frame(tmp_path):
    store = HistoryStore(tmp_path / "history", GAME.history_root)
    with Session(GAME, store, GAME.read_rom()) as session:
        session.step(8)
        session.step(8)
        node = session.checkpoint(reason="pause")

    with Session(GAME, store, GAME.read_rom(), node=node) as resumed:
...
def test_checkpoint_placement_and_presentation_cannot_change_path_identity(tmp_path):
    left, right = HistoryStore(tmp_path / 'left', GAME.history_root), HistoryStore(tmp_path / 'right', GAME.history_root)
...
def test_resume_branch_autosave_and_cache_deletion_preserve_ancestry(tmp_path):
    store = HistoryStore(tmp_path / 'history', GAME.history_root)
    with Session(GAME, store, GAME.read_rom()) as first:
        first.step(8)
        a = first.checkpoint()
        first.step(0)
        b = first.checkpoint()
    ...
    with Session(GAME, store, GAME.read_rom(), node=b) as resumed:
    ...
    with GenesisRun(GAME, GAME.read_rom()) as cold:
        path = store.flatten(x)
        cold.advance(path['end_frame'], path['events'])
        expected = cold.observable()
        cold._cache_path(store, x).unlink()
    with Session(GAME, store, GAME.read_rom(), node=x) as regenerated:
...
def test_implementation_change_invalidates_only_cache(tmp_path):
    ...
    node = store.resolve()
    path = store.flatten(node)
    with GenesisRun(GAME, GAME.read_rom()) as changed:
...
def test_valid_cache_envelope_with_invalid_native_state_cannot_move_logical_clock(tmp_path):
    ...
    with Session(GAME, store, GAME.read_rom(), node=node) as recovered:
...
def test_original_caches_survive_a_python_source_edit_but_candidate_caches_do_not(tmp_path, monkeypatch):
    ...
    node = store.resolve()
    path = store.flatten(node)
    with GenesisRun(GAME, GAME.read_rom(), 'lifecycle') as candidate:
    ...
    def edited(game, candidate='original'):
        ...
        receipt['python_modules_sha256'] = {**receipt['python_modules_sha256'], f'{GAME.package}/profile.py': '0' * 64}
        return receipt

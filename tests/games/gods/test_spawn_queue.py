...
def test_candidate_names_are_explicit():
    ...
    assert boundary.SPAWN_QUEUE_ENTRY in recovery.Candidate('camera-sprites').gate_pcs   # the combined candidate grows with every leaf
...
def test_candidate_matches_the_reference_over_real_frames_and_its_mutant_diverges():
    ...
    assert set(common['fallback_reasons']) <= recovery.ADAPTER_REFUSALS, common['fallback_reasons']

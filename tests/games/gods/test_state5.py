...
def test_candidate_name_is_explicit():
    ...
    # Retired 18 September: player_state_plan (005700) now owns the jump into state 5's
    # own handler; this candidate's own hits replace the direct gate's.
    assert boundary.STATE5_ENTRY not in recovery.Candidate('camera-sprites').gate_pcs
    assert boundary.PLAYER_STATE_ENTRY in recovery.Candidate('camera-sprites').gate_pcs
    assert recovery.Candidate('state-5-mutant-result').mutation is recovery._mutate_outcome

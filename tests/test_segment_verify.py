...
from aladdin_sega.history import HistoryStore, ROOT_ID, write_json
from aladdin_sega.history_runtime import GenesisRun
from aladdin_sega.profile import read_rom
from aladdin_sega.verification import execute_history
...
def evidence(tmp_path):
    ...
    store.set_main(node)
    rom = read_rom()
    reference = execute_history(store, rom, node=node)
...
def test_segment_from_a_retained_state_matches_the_reference_every_frame(tmp_path):
    ...
    for candidate in ('original', 'lifecycle'):
        report = segment_verify.check(fixture, frames=3, candidate=candidate, history=store.path)
...
def test_a_tampered_or_foreign_reference_is_detected_or_replaced_by_the_original(tmp_path):
    ...
    report = segment_verify.check(fixture, frames=3, candidate='original', reference=tmp_path / 'tampered.json',
                                  history=store.path)
    ...
    report = segment_verify.check(fixture, frames=3, candidate='original', reference=tmp_path / 'foreign.json',
                                  history=store.path)
    assert report['status'] == 'PASS' and report['oracle'].startswith('original run (reference is for history')
    report = segment_verify.check(fixture, frames=3, candidate='original', reference=tmp_path / 'absent',
                                  history=store.path)
...
def test_parent_fixture_reports_child_ownership_and_the_cli_exit_status(tmp_path, capsys):
    ...
    report = segment_verify.check(parent, frames=2, history=store.path)
    ...
    assert segment_verify.main([str(parent), '--frames', '2', '--history', str(store.path)]) == 0
    ...
    assert segment_verify.main([str(fixture), '--frames', '9', '--history', str(store.path), '--json']) == 0
...
def test_mid_frame_state_compares_state_and_video_first_then_the_reseeded_pcm_chain(tmp_path):
    ...
    report = segment_verify.check(mid, frames=3, candidate='original', history=store.path)
    assert report['status'] == 'PASS', report
    assert report['first_frame_pcm'] == 'not compared: state restored mid-frame'
    boundary = segment_verify.check(fixture, frames=3, candidate='original', history=store.path)
